package com.example.asus_pc.jianyue.ui;
...
//		setBackgroundColor(Color.TRANSPARENT);  // 如果是在onDraw绘制Indicator,必须设置背景，否则onDraw不执行
		...
		// 获取自定义属性 指示符的颜色
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.Indicators, 0, 0);
		mColor = ta.getColor(R.styleable.Indicators_colors, 0XFF00FF00);
		mHeight = (int) ta.getDimension(R.styleable.Indicators_heights, 2);
		ta.recycle();
